...
You always search for current, real data before making a judgment. Be concise and direct. Your output must be strict JSON only.`;
  ...
  const userPrompt = isGolf
    ? `Analyze this PGA Tour 3-ball or head-to-head matchup for betting purposes:
Player 1: ${player1}
Player 2: ${player2}
...
export default function App() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      setState(s => ({ ...s, error: "Analysis failed. Check your API key or try again.", loading: false }));
    }
  ...
  const labelStyle = { display: "block", marginBottom: 6, fontSize: 11, color: "#757575", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 600 };
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        input::placeholder, select option[value=""] { color: #555 !important; }
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "24px", marginBottom: 20 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              ...
              {tournaments.map(t => (
                <option key={t.name} value={t.name}>{t.name}{activeTab === "golf" ? ` — ${t.course}` : ` (${t.surface})`}</option>
              ))}
            </select>
          </div>
